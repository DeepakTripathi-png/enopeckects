...
class AdminController extends Controller
{
    ...
    public function dashboard_added_trips(Request $request)
    {
        try {
            ...
            $trips = [];
            foreach ($ongoing_trip as $value) {
                $trip_info = [];
    
                $tc_devices = DB::table('tc_devices')->where('uniqueid', $value->device_id)->first();
                
                if (isset($tc_devices) && !empty($tc_devices)) {
                    $positionid = $tc_devices->positionid ?? 0;
                    if (!empty($positionid)) {
                        $tc_positions = DB::table('tc_positions')->where('id', $positionid)->first();
                        $trip_info['address'] = $tc_positions->attributes->address ?? '';
                    }
                }
                
                $trips[] = [
                    '<a href="' . route('admin.trips.trip-details', ['id' => $value->id, 'trip_id' => $value->trip_id]) . '">' . ($value->trip_id ?? '') . '</a>',
                    $value->from_location_name ?? '',
                    $value->to_location_name ?? '',
                    $trip_info['address'] ?? '',
                    $value->trip_start_date ?? '',
                    $value->trip_status === 'assign' ? 'ongoing' : ''
                ];
            }
            ...
            \Log::error('Error fetching added trips: ' . $e->getMessage());
